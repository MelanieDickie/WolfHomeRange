...
Map$addLayer(Delta2014, VizParams)
...
Map$addLayer(Delta2015, VizParams)
...
Map$addLayer(Delta2016, VizParams)
...
Map$addLayer(Delta2017, VizParams)
...
Map$addLayer(Delta2018, VizParams)
...
Map$addLayer(Delta2019, VizParams)
...
Map$addLayer(Delta2020, VizParams)
...
Map$addLayer(Delta2012, VizParams)
...
Map$addLayer(Delta2011, VizParams)
...
ABWMUs2 <- st_read("G:\\My Drive\\CMU\\WolfHomeRange\\Data\\MooseDensity_OSR\\AB_WMU-OSR-Intersect.shp") 
mapview(list(ABWMUs2, RICCHR, WHECHR))
...
#BCWMUs <- st_read("G:\\My Drive\\CMU\\WolfHomeRange\\Data\\BC_MooseSurveys_2016\\MooseSurveys_2016.shp") 
#mapview(list(ABWMUs2,BCWMUs, RICCHR, WHECHR))
...
mapview(list(ABWMUs2prj, RICCHR, BCHR, WHECHR, SKHR), layer.name = c("WMU", "RICC", "BC", "WHEC", "SK"))
mapview(ABWMUs2prj)
...
write.csv("ABWMUs2prjatt, G:\\My Drive\\CMU\\WolfHomeRange\\Data\\MooseDneistyEVI.csv")

plot(ABWMUs2prjatt$density_avg ~ ABWMUs2prjatt$EVIMean)

require(ggplot2)
ggplot(data=ABWMUs2prjatt, aes(x=(EVIMean), y=(density_avg))) +
  geom_point(size=2) +
  theme_bw() +  
  xlab(expression(paste("Mean dEVI"))) +
  ...
  labs(colour="Season", shape = "Study Area", linetype="Season")

######## Model #############
hist(log(ABWMUs2prjatt$density_avg))
...
write.csv(moosemodeltbl, "G:\\My Drive\\CMU\\WolfHomeRange\\MooseModelTable.csv")
...
# 
# 
# moosepredex <- expand.grid(EVIMean=seq(minEVIMoose, maxEVIMoose, by=50), year = "2014", method = "distance", region = c("boreal", "parkland"))
# moosepreddensities <-  exp(predict(moosemodel,moosepredex, interval = "confidence"))
# moosepredex<-cbind(moosepredex, moosepreddensities)
# 
# require(ggplot2)
# ggplot(data=moosepredex, aes(x=(EVIMean), y=(fit), shape=region, colour=region)) +
#   geom_line(size=1) +
#   geom_line(aes(y = lwr, colour=region),linetype = "dashed")+
#   geom_line(aes(y = upr, colour=region), linetype = "dashed")+
#   ##Plot raw data
#   geom_point(data=ABWMUs2prjatt, aes(x=(EVIMean), y =density_avg, shape=region), size=2)+
#   theme_bw() +  
#   xlab(expression(paste("delta EVI"))) +
#   ylab(expression(paste("Moose density (/ ", km^2, ")"))) +
#   theme(axis.text.x = element_text(size=12), axis.title = element_text(size=14) ) + 
#   theme(axis.text.y = element_text(size=12)) + 
#   theme(panel.grid.minor=element_blank(), panel.grid.major=element_blank()) + 
